from PyQt5.QtWidgets import QWidget, QVBoxLayout, QTabWidget, QGroupBox, QFormLayout, QLineEdit, \
    QPushButton, QLabel, QSpinBox, QComboBox, QHBoxLayout, QTableWidget, \
...
class CombinedFrameCreator(QWidget):
    ...
    def setup_make_frame_tab(self, tab):
        ...
        self.horizontal_option_combo.addItems(['Horizontal Normal', 'Horizontal with Flip', 'Half horizontal with Flip'])
    ...
    def setup_table(self):
        ...
        items = []
        try:
            with open('Sizes/dimensions.csv', 'r') as file:
                reader = csv.reader(file)
                next(reader)  # Skip the header row if there is one
                for row in reader:
                    if len(row) >= 2:  # Ensure at least height and width columns
                        items.append((row[0], row[1]))
        except FileNotFoundError:
            print("CSV file not found. Using default values.")


        # Set the number of rows based on items
        self.table.setRowCount(len(items))

        # Populate the table
        for row, (height, width) in enumerate(items):
            item_height = QTableWidgetItem(height)
            item_height.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
            self.table.setItem(row, 0, item_height)

            item_width = QTableWidgetItem(width)
            item_width.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
            self.table.setItem(row, 1, item_width)
        ...
        # Buttons for resizing and processing
